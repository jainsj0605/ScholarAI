...
def clean_math_output(text: str) -> str:
    """
    Robust LaTeX cleaning utility to fix delimiters and bizarre separators.
    Ensures formulas render correctly in Streamlit's markdown.
    """
    ...
    # 2. REMOVE BIZARRE SEPARATORS: Handle ";=;", ";+;", etc. generated by some models
    # This matches patterns like ;=; or ; = ; or ;+;
    text = re.sub(r'\s*;\s*=\s*;\s*', ' = ', text)
    text = re.sub(r'\s*;\s*\+\s*;\s*', ' + ', text)
    text = re.sub(r'\s*;\s*-\s*;\s*', ' - ', text)
    text = re.sub(r'\s*;\s*/\s*;\s*', ' / ', text)
    text = re.sub(r'\s*;\s*\*\s*;\s*', ' * ', text)
    text = re.sub(r'\s*;\s*approx\s*;\s*', r' \\approx ', text)
    text = re.sub(r'\s*;\s*dot\s*;\s*', r' \\cdot ', text)

    # 3. FIX OVER-ESCAPED BACKSLASHES: \\frac -> \frac
    text = text.replace('\\\\', '\\')

    # 4. CLEAN UP SPACING and NOISE inside math blocks
    # Normalize whitespace and remove common model artifacts like "!" or trailing commas
    def clean_math_block(match):
        inner = match.group(1).strip()
        # Remove literal "!" noise often found in model-generated math
        inner = inner.replace('!', '')
        # Remove trailing commas/periods often trapped inside the AI's math delimiters
        inner = re.sub(r'[,.]$', '', inner)
        # Normalize multiple spaces into single space
        inner = re.sub(r'\s+', ' ', inner).strip()
        
        # Determine if we use $ or $$ (based on the original match)
        prefix = match.group(0)[0:2]
        if prefix != '$$':
            return f'${inner}$'
        return f'$$\n{inner}\n$$'
    ...
    # Clean Inline Math $...$ (careful not to double-match step above)
    text = re.sub(r'(?<!\$)\$(?!\$)(.*?)(?<!\$)\$(?!\$)', clean_math_block, text, flags=re.DOTALL)
